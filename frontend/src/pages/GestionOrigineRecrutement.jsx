...
export default function GestionOrigineRecrutementPage() {
  ...
  const loadList = async () => {
    ...
    setLoadingList(true)
    try {
      const res = await apiJson('/api/origineRecrutement')
  ...
  const onDelete = async (id) => {
    ...
    setSuccess('')
    try {
      await apiJson(`/api/origineRecrutement/${id}`, { method: 'DELETE' })
      await loadList()
    } catch (err) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await apiJson(isEdit ? `/api/origineRecrutement/${editingId}` : '/api/origineRecrutement', {
        method: isEdit ? 'PUT' : 'POST',
        body: { libelle },
      })
      setSuccess(isEdit ? 'Origine de recrutement modifiée avec succès.' : 'Origine de recrutement enregistrée avec succès.')
      closeModal()
      await loadList()
    } catch (err) {
    ...
    <div className="app-page">
      ...
      <div className="app-container">
        <div className="app-card">
          <h2 style={{ marginTop: 0 }}>Origine Recrutement</h2>
          ...
          <div style={{ marginTop: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <h3 style={{ margin: 0 }}>Liste Origine Recrutement</h3>
              <button
                type="button"
        ...
        <div className="modal-backdrop" onClick={closeModal}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div className="modal-title">{editingId != null ? 'Modifier Origine Recrutement' : 'Nouvelle Origine Recrutement'}</div>
